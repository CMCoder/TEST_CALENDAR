...
- (instancetype)init
{
    ...
    self->_calendarAppearance = [LECCalendarAppearance new];
    self->_dataCache = [LECCalendarDataCache new];
...
- (void)setMenuMonthsView:(LECCalendarMenuView *)menuMonthsView
{
    [self->_menuMonthsView setDelegate:nil];
...
- (void)setContentView:(LECCalendarContentView *)contentView
{
    [self->_contentView setDelegate:nil];
...
{
    NSAssert(currentDate, @"LECCalendar currentDate cannot be null");
...
{
    if(self.calendarAppearance.isWeekMode){
        NSLog(@"LECCalendar loadNextMonth ignored");
        return;
    }
...
{
    if(self.calendarAppearance.isWeekMode){
        NSLog(@"LECCalendar loadPreviousMonth ignored");
        return;
    }
